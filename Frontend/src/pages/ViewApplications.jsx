    ...
    return applicants ? (
        applicants.length === 0 ? (
            <div className='flex items-center justify-center h-[70vh]'>
                <p className='text-xl sm:text-2xl'>No Applicatios Available</p>
            </div>
        ) : (
                        ...
                        {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
                            <tr key={index} className='text-gray-700'>
                                ...
                                <td className='py-2 px-4 border-b'>
                                    <a className='bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex gap-2 items-center'
                                        href={applicant.userId.resume} target="_blank" rel="noopener noreferrer">
                                        Resume
                                ...
                                <td className='py-2 px-4 border-b text-center font-bold'>
                                    <span className={`px-2 py-1 rounded 
                                        ${applicant.status === 'Accepted' ? 'bg-green-100 text-green-600' :
                                            applicant.status === 'Rejected' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
