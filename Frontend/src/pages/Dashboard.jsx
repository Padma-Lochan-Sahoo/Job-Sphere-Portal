...
const Dashboard = () => {
    ...
    // Default page when ever recruiter logs in
    useEffect(() => {
        if(companyData){
  ...
  return (
    <div className='min-h-screen'>
        {/* Navbar for Recurtier Panel */}
        <div className='shadow py-4'>
            <div className='px-5 flex justify-between items-center'>
                <img 
                onClick={(e)=>navigate('/')}
                className='max-sm:w-32 cursor-pointer' src={assets.logo} alt="" />
                { companyData && (
                    <div className='flex items-center gap-3'>
                    <p
                    className='max-sm:hidden'
                    >Welcome ,  {companyData.name}</p>
        ...
        <div className='flex items-start'>
            ...
            <div className='inline-block min-h-screen border-r-2'>
                <ul className='flex flex-col text-gray-800 items-start pt-5'>
                    <NavLink
